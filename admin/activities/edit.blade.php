                ...
                <form action="{{ route('admin.activities.update', $activity) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        <div>
                            ...
                            <x-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name', $activity->name)" required autofocus />
                            @error('name')
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="4" class="w-full border-gray-300 dark:border-gray-700 dark:bg-gray-700 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">{{ old('description', $activity->description) }}</textarea>
                            @error('description')
                                <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <x-input id="location" class="block mt-1 w-full" type="text" name="location" :value="old('location', $activity->location)" />
                            @error('location')
                                <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            @if($activity->image)
                                <div class="mb-2">
                                    <img src="{{ Storage::url($activity->image) }}" alt="{{ $activity->name }}" class="h-32 w-auto rounded">
                                </div>
                            @endif
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <x-input id="start_date" class="block mt-1 w-full" type="datetime-local" name="start_date" :value="old('start_date', $activity->start_date ? $activity->start_date->format('Y-m-d\TH:i') : '')" />
                                @error('start_date')
                                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            <div>
                                ...
                                <x-input id="end_date" class="block mt-1 w-full" type="datetime-local" name="end_date" :value="old('end_date', $activity->end_date ? $activity->end_date->format('Y-m-d\TH:i') : '')" />
                                @error('end_date')
                                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <select id="activity_type" name="activity_type" class="w-full border-gray-300 dark:border-gray-700 dark:bg-gray-700 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                    <option value="public" {{ old('activity_type', $activity->activity_type) == 'public' ? 'selected' : '' }}>Public</option>
                                    <option value="private" {{ old('activity_type', $activity->activity_type) == 'private' ? 'selected' : '' }}>Private</option>
                                </select>
                                @error('activity_type')
                                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                                @enderror
